...
# Step 1 — Define a country-name cleaning helper (reuse everywhere)
...
# Step 2 — Clean country names and assign ISO3 codes
...
Data_complete_iso <- Data_complete %>%
  mutate(
    ...
    ## ISO3 codes
    ISO3_host = countrycode(
      host_country_clean,
      origin = "country.name",
      destination = "iso3c"
    ),
    ISO3_proponent = countrycode(
      proponent_country_clean,
      origin = "country.name",
      destination = "iso3c"
    ),

    ## Handle known legacy mismatches (important)
    ISO3_host = case_when(
      ISO3_host == "COD" ~ "ZAR",  # DR Congo (Zaire in WGI / legacy data)
      ISO3_host == "ROU" ~ "ROM",  # Romania legacy
      TRUE ~ ISO3_host
    ),
    ISO3_proponent = case_when(
      ISO3_proponent == "COD" ~ "ZAR",
      ISO3_proponent == "ROU" ~ "ROM",
      TRUE ~ ISO3_proponent
    )
  )

# Step 3 — Assign continents from ISO3 codes #
...
Data_complete_iso <- Data_complete %>%
  mutate(
    ...
    ## Legacy ISO3 (ONLY for WGI / legacy datasets)
    ISO3_host = case_when(
...
#### Step 1 Define country groups by ISO3 ####

north_america_iso3 <- c(
  "USA",  # United States
  "CAN"   # Canada
)
...
Data_complete_iso <- Data_complete_iso %>%
  mutate(
    ...
    proponent_continent_iso = case_when(
      ...
      ISO3_proponent_modern %in% central_america_iso3 ~ "Central America",
...
# Step 4 - assign continents using modern ISO only
